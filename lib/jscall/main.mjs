...
let raw_stdout = console.log
let locked = true
let waiting_queue = []

const stdout_puts = (message) => {
    if (locked) {
...
const unlock = () => {
    if (!locked) {
        throw new Error("fail to unlock -- it has not been locked")
    } else if (waiting_queue.length > 0) {
        const next_message = waiting_queue.shift()
        raw_stdout(next_message)
    } else {
        locked = false
    }
}
...
export const start = async (stdin, use_stdout) => {
    if (use_stdout)
        ...
        raw_stdout = (m) => stdin.puts(m)  // on browser

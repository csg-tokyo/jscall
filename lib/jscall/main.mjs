...
const funcall_from_ruby = cmd => {
    // const message_id = cmd[1]  // it must be undefined
...
const reply = (value, sync_mode) => {
    if (sync_mode && value instanceof Promise)
        ...
        try {
            const cmd = reply_with_piggyback([cmd_reply, undefined, encode_obj(value)])
...
const reply_error = e => {
    const cmd = reply_with_piggyback([cmd_reply, undefined, encode_error(e)])

    ...
    get(obj, name) {
        if (name === '__self__')
            ...
            return (...args) => {
                ...
                return funcall_to_ruby(exported.export_remoteref(obj), name, args)
            }
    }
    apply(obj, self, args) {
        // this returns Promise
        return funcall_to_ruby(exported.export_remoteref(obj), 'call', args)
    }
}
...
export const funcall_from_ruby = cmd => {
    ...
    if (debug_level >= 10)
        console.error(`RubyToJS> ${name} ${cmd[1]} ${cmd[2]}`)
